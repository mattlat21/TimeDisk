...
{
    char buf[16];
    ui_common_format_mm_ss(buf, sizeof(buf), s_duration_sec);
...
{
    const ui_theme_t *t = ui_theme_get();
    s_scr_duration = ui_common_create_screen();
    screens[UI_SCREEN_TIMER_DURATION] = s_scr_duration;
    s_duration_sec = app_config_get()->timer_duration_sec;

    ui_common_create_title(s_scr_duration, "Set Duration");

    lv_obj_t *box = ui_common_create_purple_box(s_scr_duration, 400, 120, 160, 200, false);
    ...
    lv_obj_t *back = ui_common_create_side_btn(s_scr_duration, true, 36, 330, NULL);
    lv_obj_add_event_cb(back, duration_back_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_t *next = ui_common_create_side_next(s_scr_duration, UI_DISP - 36 - 64, 330);
...
static void build_style(lv_obj_t *screens[UI_SCREEN_COUNT])
{
    ...
    lv_obj_t *back = ui_common_create_side_btn(s_scr_style, true, 36, 330, NULL);
    lv_obj_add_event_cb(back, style_back_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_t *next = ui_common_create_side_next(s_scr_style, UI_DISP - 36 - 64, 330);
...
{
    const ui_theme_t *t = ui_theme_get();
    s_scr_triggered = ui_common_create_screen();
    ...
    lv_obj_t *ok = ui_common_create_side_btn(s_scr_triggered, false, UI_DISP - 36 - 64, 330, NULL);
    lv_obj_add_event_cb(ok, triggered_ok_cb, LV_EVENT_CLICKED, NULL);
}

static void build_confirm(lv_obj_t *screens[UI_SCREEN_COUNT])
{
    s_scr_confirm = ui_common_create_screen();
    screens[UI_SCREEN_CONFIRM_END_TIMER] = s_scr_confirm;

    ui_common_create_title(s_scr_confirm, "Are you sure?");

    lv_obj_t *no = ui_common_create_side_btn(s_scr_confirm, true, 36, 330, "No");
    lv_obj_add_event_cb(no, confirm_no_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_t *yes = ui_common_create_side_btn(s_scr_confirm, false, UI_DISP - 36 - 64, 330, "Yes");
    lv_obj_add_event_cb(yes, confirm_yes_cb, LV_EVENT_CLICKED, NULL);
}

extern "C" void ui_screen_timer_build(lv_obj_t *screens[UI_SCREEN_COUNT])
{
...
extern "C" void ui_screen_timer_on_show(ui_screen_id_t id)
{
    char buf[16];
    app_runtime_t *rt = app_runtime_get();
    ui_common_format_mm_ss(buf, sizeof(buf), rt->active_timer_remaining_sec);
...
extern "C" void ui_screen_timer_tick(void)
{
    char buf[16];
    app_runtime_t *rt = app_runtime_get();
    if (!rt->timer_running) {
...
extern "C" uint32_t ui_screen_duration_get_sec(void)
{
    return s_duration_sec;
}

extern "C" uint8_t ui_screen_style_get_id(void)
{
    return s_style_id;
}
