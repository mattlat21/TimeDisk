                                ...
                                const char *title, int idx)
{
    ...
    ui_common_create_title(scr, title);

    lv_obj_t *box = ui_common_create_purple_box(scr, 400, 100, 160, 220, false);
    ...
    lv_obj_t *back = ui_common_create_side_btn(scr, true, 36, 330, NULL);
    lv_obj_add_event_cb(back, back_cb, LV_EVENT_CLICKED, (void *)(uintptr_t)id);
    lv_obj_t *next = ui_common_create_side_next(scr, UI_DISP - 36 - 64, 330);
    lv_obj_add_event_cb(next, next_cb, LV_EVENT_CLICKED, (void *)(uintptr_t)id);
}

extern "C" void ui_screen_schedule_build(lv_obj_t *screens[UI_SCREEN_COUNT])
{
...
extern "C" uint32_t ui_screen_schedule_get_sec(void)
{
    return s_wizard_vals[0];
}

extern "C" void ui_screen_schedule_set_sec(uint32_t sec)
{
    s_wizard_vals[0] = sec;
}
