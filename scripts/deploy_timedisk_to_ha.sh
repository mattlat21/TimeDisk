#!/usr/bin/env bash
#
# Deploy the TimeDisk Home Assistant custom component via Samba (SMB/CIFS).
#
...
#   <share root>/custom_components/timedisk/
# which is the same as /config/custom_components/timedisk/ on HA.
#
# Clean updates:
# - Everything under custom_components/timedisk/ on the share is removed, then replaced
...
# SAFETY
#
# - Only the directory custom_components/timedisk under the mounted share is wiped.
# - Before rm, we resolve realpath and require the last path component to be `timedisk`.
...
#   ha core restart
# on HA_SSH_USER@HA_HOST. Requires SSH access + sshpass if you use SSH password.
#
# --------------------------------------------------------------------------------------
# USAGE
#
#   SMB_USER=... SMB_PASSWORD=... ./scripts/deploy_timedisk_to_ha.sh
#   ./scripts/deploy_timedisk_to_ha.sh --smb-user ha --smb-password 'secret'
#
# ENV FILE (recommended):
#   cp scripts/deploy_timedisk_to_ha.env.example scripts/deploy_timedisk_to_ha.env
#   # edit SMB_USER / SMB_PASSWORD (use quotes if password contains & etc.)
#   ./scripts/deploy_timedisk_to_ha.sh --dry-run
#   Override file path: DEPLOY_ENV_FILE=/path/to/.env ./scripts/deploy_timedisk_to_ha.sh
#   Or: ./scripts/deploy_timedisk_to_ha.sh --env-file /path/to/.env --dry-run
#
#   ./scripts/deploy_timedisk_to_ha.sh --restart
...
# scripts/deploy_timedisk_to_ha.env next to this script.
ENV_FILE_TO_SOURCE=""
_pre_args=("$@")
...
if [[ -z "${ENV_FILE_TO_SOURCE}" ]]; then
  ENV_FILE_TO_SOURCE="${DEPLOY_ENV_FILE:-${SCRIPT_DIR}/deploy_timedisk_to_ha.env}"
fi
...
# Paths (local repo)
# -------------------------
LOCAL_SRC_DIR="integrations/home-assistant/custom_components/timedisk/"

# Relative path on the config share (share root == /config).
SHARE_REL_DEST="custom_components/timedisk"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: deploy_timedisk_to_ha.sh [OPTIONS]

Deploy integrations/home-assistant/custom_components/timedisk/ to the HA config
share via SMB (Samba add-on).

Options:
  --env-file PATH         Load SMB_* (and optional SSH_*) from a file (also auto-loads
                          scripts/deploy_timedisk_to_ha.env if it exists).
...
Examples:
  ...
  SMB_USER=ha SMB_PASSWORD='secret' ./scripts/deploy_timedisk_to_ha.sh --dry-run
  ./scripts/deploy_timedisk_to_ha.sh --smb-user ha --smb-password 'secret'
EOF
      exit 0
      ;;
...
MNT="$(mktemp -d "${TMPDIR:-/tmp}/timedisk_smb.XXXXXX")"
...
if [[ "$(basename "$RESOLVED")" != "timedisk" ]]; then
  echo "ERROR: Refusing to continue; resolved path does not end with timedisk: $RESOLVED"
  exit 1
fi
